 ...
 * @version $Id: $
 */
public class CreateCalendarEventsJobImpl extends AbstractHecQuartzJobImpl {

    private static Log log = LogFactory.getLog(CreateCalendarEventsJobImpl.class);
    private static ResourceLoader rb = new ResourceLoader("ca.hec.sakai.jobs.bundle.CMJobMessages");
	...
	public void execute(JobExecutionContext arg0) {
		...
		List<HecEvent> eventsAdd = jdbcTemplate
				.query(select_from
						+ "where (EVENT_ID is null and (STATE is null or STATE <> 'D'))"
						+ order_by, new HecEventRowMapper());

		// keep track of the last event's site id, calendar and courseOffering,
		// so we can use the calendar if it was already found
		Calendar calendar = null;
		String previousSiteId = "";
		boolean calendarFound = false;
		CourseOffering courseOffering = null;

		String classSection, providerId, courseOfferingId = "";
		Set<String> groupIds = null;
		Set<Group> selectedGroupId = null;
		Site site;

		log.info("loop and add " + eventsAdd.size() + " events");
		for (HecEvent event : eventsAdd) {
			classSection = event.getSection();

			String siteId = getSiteId(event.getCatalogNbr(),
			...
			if (!siteId.equals(previousSiteId)) {
				...
				try {
					...
					courseOfferingId = event.getCatalogNbr()
							+ event.getSessionId() + event.getSessionCode();

					courseOffering = cmService
							.getCourseOffering(courseOfferingId);

				} catch (IdNotFoundException e) {
					log.debug("Site " + siteId
							+ " not associated to course management");
				} catch (IdUnusedException e) {
					log.debug("Site or Calendar for " + siteId
							+ " does not exist");
			...
			providerId = event.getCatalogNbr() + event.getSessionId()
					+ event.getSessionCode() + event.getSection();

			groupIds = authzGroupService.getAuthzGroupIds(providerId);
			selectedGroupId = new HashSet<Group>();

			try {
				site = siteService.getSite(siteId);
				// remove group for whole site
				for (String group : groupIds) {
					if (group.contains("group"))
						selectedGroupId.add(site.getGroup(group));
				}
			} catch (IdUnusedException e) {
				//no log needed
				//log.debug("Site for " + siteId + " does not exist");
			}

			// only attempt event creation if the calendar was found
			if (courseOffering != null && calendarFound) {

				// don't bother adding the events if this is an MBA site
				// (ZCII-1495)
				// and the event is not a final or mid-term (intratrimestriel)
				// exam
				if (!courseOffering.getAcademicCareer().equals(CAREER_MBA)
					...
					eventId = createCalendarEvent(
							...
							event.getStartTime(),
							event.getEndTime(),
							getEventTitle(siteId, event.getExamType(),
									event.getSequenceNumber()),
							getType(event.getExamType()), event.getLocation(),
							event.getDescription(), selectedGroupId);
				} else {
					log.debug("Skipping event creation: "
							+ getEventTitle(siteId, event.getExamType(),
									event.getSequenceNumber()) + " for site "
							+ siteId
							+ " (course is MBA and event is not an exam)");
				}
			}

			// clear the state in HEC_EVENT regardless
			clearHecEventState(eventId, event.getCatalogNbr(),
					event.getSessionId(), event.getSessionCode(),
					event.getSection(), event.getExamType(),
					event.getSequenceNumber());
		...
		List<HecEvent> eventsUpdate = jdbcTemplate.query(select_from
				+ "where (STATE = 'M' or STATE = 'D')" + order_by,
		...
		for (HecEvent event : eventsUpdate) {
			...
			if (courseOffering != null && calendarFound) {
				...
				// don't bother modifying the events if this is an MBA site
				// (ZCII-1495)
				// and the event is not a final or mid-term (intratrimestriel)
				// exam
				if (!courseOffering.getAcademicCareer().equals(CAREER_MBA)
					...
					updateSuccess = updateCalendarEvent(calendar,
							event.getEventId(), event.getState(),
							event.getStartTime(), event.getEndTime(),
							event.getLocation(), event.getDescription());
				} else {
					log.debug("Skipping event update: "
							+ getEventTitle(siteId, event.getExamType(),
									event.getSequenceNumber()) + " for site "
							+ siteId
							+ " (course is MBA and event is not an exam)");
			...
			if (event.getState().equals("M")) {
				clearHecEventState(event.getEventId(), event.getCatalogNbr(),
						event.getSessionId(), event.getSessionCode(),
						event.getSection(), event.getExamType(),
						event.getSequenceNumber());
				...
				deleteHecEvent(event.getCatalogNbr(), event.getSessionId(),
						event.getSessionCode(), event.getSection(),
						event.getExamType(), event.getSequenceNumber());
		...
		logoutFromSakai();
		log.info("added: " + addcount + " updated: " + updatecount
				+ " deleted: " + deletecount);
	} // execute
	...
	private String createCalendarEvent(Calendar calendar, Date startTime, Date endTime, String title, String type, String location, String description, Collection section)
	{
		CalendarEvent event;
		try {
