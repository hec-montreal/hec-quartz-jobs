...
public class GenericExamensMapFactory {
    ...
    public static void addExamens(String completeFileName,
	...
	/*
	 * ExamensMap map; try { map = getInstance(dataDir);
	 * print("Mise a jour de la map..."); } catch (FileNotFoundException e)
	 * { print("La map n'a pas ete trouvee, on la recree au complet."); map
	 * = new ExamensMap(); } catch (IOException e) {
	 ...
	 * 2003;2062;1;2006-07-05;18:30;21:30;PricewaterhouseCoopers;�difice
	 * C�te-Sainte-Catherine;TOUS;FIN
	 * 2004;2062;1;2006-06-01;18:30;20:30;H�l�ne-Desmarais;�difice
	 * C�te-Sainte-Catherine;A � HALI;INTR
	 * 2004;2062;1;2006-06-01;18:30;20:30;Xerox Canada;�difice
	 * C�te-Sainte-Catherine;HALJ � Z;INTR
	 */
	String numeroHEL, session, periode, date, hrDebut, hrFin, salle, edifice, repartition, type;
	...
	while ((buffer = breader.readLine()) != null) {
		...
		salle = tokenizer.nextToken(); // TODO: penser aux entrees des
					       // DF1...
		edifice = tokenizer.nextToken();
		repartition = tokenizer.nextToken();
		type = tokenizer.nextToken();
	    } catch (java.util.NoSuchElementException e) {
		log.info("GenericExamensMapFactory: NoSuchElementException: "
			+ buffer);
	     ...
	     * pourrais consid�rer ceci: Je pourrais utiliser comme cl�:
	     * numeroHEL + session + periode + 1 (si c la premi�re entr�e
	     * rencontr�e) et dans ce cas je v�rifie si on en a d�j� une dans la
	     * map et si c la m�me r�partition. if(map.containsKey(matricule)) {
	     * entry = map.get(matricule); } else { entry = new
	     * ExamensMapEntry(matricule); map.put(entry); }
	     */

	    DetailCoursMapEntry cours =
		    detailCoursMap.get(numeroHEL, session, periode);
	    if (cours == null) {
	    ...
	    ExamenMapEntry examen =
		    new ExamenMapEntry(cours, date, hrDebut, hrFin, salle,
	...
	if (erreurs.length() > 0) {
	    erreurs.insert(
		    0,
		    "Les lignes suivantes ont g�n�r� une NoSuchElementException.\n"
			    + "Les examens correspondants n'ont donc pas �t� cr��.\n\n");
	    /*
	     * try { ca.hec.mail.Mailer.postMail("remi.saias@hec.ca",
	     * "Probl�me avec l'extract des examens", erreurs.toString(),
	     * "remi.saias@hec.ca"); } catch (javax.mail.MessagingException e) {
    ...
    /*
     * public static ExamensMap getInstance(String dataDir) throws IOException {
     * return getInstance(dataDir, DEFAULT_BASE_NAME); } public static
     * ExamensMap getInstance(String dataDir, String mapName) throws IOException
