...
public class HecExamExceptionGroupImpl implements HecExamExceptionGroup {
	...
	sqlService.dbWrite(
		"insert into HEC_CAS_SPEC_EXM (STRM, EMPLID , NAME, N_PRCENT_SUPP, ACAD_CAREER, SUBJECT, CATALOG_NBR, CLASS_SECTION, N_EMAIL_ADJ_PRINC, N_LISTE_EMAIL_ADJ, N_LISTE_EMAIL_PROF, N_EMAIL_COORD, STATE)"
			+ "select STRM, EMPLID , NAME, N_PRCENT_SUPP, ACAD_CAREER, SUBJECT, CATALOG_NBR, CLASS_SECTION, N_EMAIL_ADJ_PRINC, N_LISTE_EMAIL_ADJ, N_LISTE_EMAIL_PROF, N_EMAIL_COORD, 'A' "
	...
	sqlService.dbWrite(
		"update HEC_CAS_SPEC_EXM set STATE = 'D' where (EMPLID, NAME, CATALOG_NBR, STRM, CLASS_SECTION, SUBJECT, N_PRCENT_SUPP) not in "
			...
			+ "and (STATE is null or STATE != 'E') and STRM=" + sessionId);

	log.info("Traiter les changements de courriels");
	sqlService.dbWrite(
		"update HEC_CAS_SPEC_EXM t1 set ( N_EMAIL_ADJ_PRINC, N_LISTE_EMAIL_ADJ, N_LISTE_EMAIL_PROF, N_EMAIL_COORD) ="
			+ "                         (select N_EMAIL_ADJ_PRINC, N_LISTE_EMAIL_ADJ, N_LISTE_EMAIL_PROF, N_EMAIL_COORD"
			...
			+ "and (STATE is null or STATE != 'D') and STRM=" + sessionId);
